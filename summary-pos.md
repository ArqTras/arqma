...
| **`pulse_proposal`** | **`blk`**, optional **`rh`** | Per-sender rate limit: combined **`pulse_proposal`** + **`pulse_vote`** capped by **`arqma::pulse_wire::PULSE_QUORUM_PEER_RATE_*`** (hex sender key); **`ok=0`** + **`e`** `pulse proposal rate limit` when exceeded. Then: parse blob; when **`FORK_ACTIVE`** && block **`major_version >= network_version_20_pos`**, **`Blockchain::verify_pulse_fork_block_rules`**. If **`bh`** was **already accepted** within the dedup window, reply **`ok=1`** + **`bh`** only. Otherwise reply **`bh`** or **`ok=0`**; on success **`mark_seen(bh)`** then relay as above. |
