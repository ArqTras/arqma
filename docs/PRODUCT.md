...
| `arqma-storage` | HTTP Storage Server (SN uptime probe + KV / swarm list) |
| `arqma-router` | Privacy-router process (HTTP status; separate from `arqmad`) |
| `arqma-msg` | CLI messenger: identity, sealed envelope, store/retrieve via storage |
