...
|-------|--------|-----------------|--------------------------------------|
| 3 MQ feature parity | Foundation + native transport | facade, ACL, command registry, `SocketStack` (`transport=arqmq` when flagged); mesh still SNNetwork | Dual-run mesh parity + default cutover |
...
| 10 Testing | Foundation done | curated suite **535** green (Linux/macOS CI) | `core_tests` under `BUILD_INTEGRATION_TESTS`; remaining API-drift fixtures |
