...
struct SNNWrapper
{
  ...
  SNNWrapper(cryptonote::core& core, Args&&... args) : snn{std::forward<Args>(args)...}, core{core}
  {}
...
{
  el::base::Writer(easylogging_level(level), el::Color::Default, file, line, ELPP_FUNC,
                   el::base::DispatchAction::NormalLog)
          .construct(ARQMA_DEFAULT_LOG_CATEGORY)
      << msg;
}
...
void* new_snnwrapper(cryptonote::core& core, const std::string& bind)
{
  ...
  auto allow = [&sn_list = core.get_service_node_list()](const std::string& ip, const std::string& x25519_pubkey_str) {
    ...
    if (decision == IncomingCurveDecision::ServiceNode) {
      MINFO("Accepting incoming SN connection authentication from ip/x25519/pubkey: " << ip << "/" << x25519_pubkey
                                                                                      << "/" << pubkey);
  ...
  if (arqmq::native_transport_active()) {
    MINFO("Arq-Net dual-run active: native transport=" << arqmq::transport_name() << ", peer mesh="
                                                       << arqmq::mesh_transport_name() << " (compatibility mode)");
  }
...
public:
  ...
  peer_info(SNNWrapper& snw, quorum_type q_type, std::shared_ptr<const quorum>& quorum, bool opportunistic = true,
            exclude_set exclude = {})
      : peer_info(snw, q_type, &quorum, &quorum + 1, opportunistic, std::move(exclude))
  {}

  template <typename QuorumIt>
  peer_info(SNNWrapper& snw, quorum_type q_type, QuorumIt qbegin, QuorumIt qend, bool opportunistic = true,
      ...
      : snn{snw.snn}
  {
    ...
    snw.core.get_service_node_list().for_each_service_node_info_and_proof(
        need_remotes.begin(), need_remotes.end(), [this](const auto& pubkey, const auto& info, const auto& proof) {
          if (info.is_active() && proof.pubkey_x25519 && proof.arqnet_port && proof.public_ip)
            remotes.emplace(pubkey,
                            std::make_pair(proof.pubkey_x25519,
                                           "tcp://" + epee::string_tools::get_ip_string_from_int32(proof.public_ip) +
                                               ":" + std::to_string(proof.arqnet_port)));
        });
...
private:
    ...
    for (QuorumIt qit = qbegin; qit != qend; ++i, ++qit) {
      ...
      for (int j : quorum_incoming_conns(my_position[i], validators.size())) {
        if (add_peer(validators[j], false))
          MTRACE("Optional opportunistic relay with quorum " << (i == 0 ? "Q" : "Q'") << " to service node "
                                                             << validators[j]);
      }
  ...
  void relay_to_peers_impl(const std::string& cmd, std::array<send_option::serialized, N> relay_data,
                           std::index_sequence<I...>)
  {
    for (auto& peer : peers) {
      MTRACE("Relaying " << cmd << " to peer " << as_hex(peer.first)
...
{
  bt_dict result{
      {"v", vote.version},        {"t", static_cast<uint8_t>(vote.type)},
      {"h", vote.block_height},   {"g", static_cast<uint8_t>(vote.group)},
      {"i", vote.index_in_group}, {"s", get_data_as_string(vote.signature)},
  };
...
quorum_vote_t deserialize_vote(const bt_value& v)
{
  ...
  if (vote.type == quorum_type::checkpointing) {
    ...
    if (bh.size() != sizeof(vote.checkpoint.block_hash.data))
...
void relay_obligation_votes(void* obj, const std::vector<service_nodes::quorum_vote_t>& votes)
{
  ...
  for (const auto& vote : votes) {
    if (vote.type != quorum_type::obligations) {
      MERROR("Internal logic error: arqnet asked to relay a "
             << vote.type << " vote, but should only be called with obligations votes");
      continue;
    }
  ...
  for (auto& [quorum_key, quorum_votes] : votes_by_quorum) {
    ...
    if (vote_index % 50 == 0 || vote_index == votes.size())
      MDEBUG("Processing vote group " << vote_index - quorum_votes.size() + 1 << "-" << vote_index << " of "
                                      << votes.size() << " (quorum at height " << block_height << ")");

    auto quorum = snw.core.get_service_node_list().get_quorum(q_type, block_height);
    if (!quorum) {
      MWARNING("Unable to relay " << quorum_votes.size() << " votes: no " << q_type << " quorum available for height "
                                  << block_height);
      continue;
    }
    ...
    if (quorum_voters.size() < service_nodes::min_votes_for_quorum_type(q_type)) {
      MWARNING("Invalid vote relay: " << q_type << " quorum @ height " << block_height
                                      << " does not have enough validators (" << quorum_voters.size()
